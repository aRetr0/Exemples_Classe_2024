...
public class ProducteCompost extends ProducteAbstracte {
    ...
    public boolean addProduct(ProducteAbstracte nou) {
        ...
        if (this.isRepe(nou)) return false;
        if (nou instanceof ProducteCompost && ((ProducteCompost) nou).isRepe(this)) return false;
        productes.add(nou);
        return true;
    }
    ...
    private boolean isRepe(ProducteAbstracte nou) {
        for (ProducteAbstracte item : productes) {
            if (item == nou && item instanceof ProducteCompost) return true;
            if (item instanceof ProducteCompost && ((ProducteCompost) item).isRepe(nou)) return true;
